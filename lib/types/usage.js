 ...
 */
export function priceCall(price, usage, epochMs) {
    ...
    return (miss * basis.cacheMissInputPerM
        + hit * basis.cacheHitInputPerM
        + output * basis.outputPerM) / 1_000_000;
}
/**
 ...
 */
export function hourBucketOf(formatter, epochMs) {
    ...
    const field = (type) => {
        ...
        if (part === undefined)
            throw new Error(`hour formatter has no "${type}" part`);
 ...
 */
export function buildHourFormatter(timeZone) {
...
function foldEventRows(rows, range) {
    ...
    for (const event of rows) {
        if (!inRange(event.time, range))
            continue;
        if (firstEventAt === 0 || event.time < firstEventAt)
            firstEventAt = event.time;
        if (event.time > lastEventAt)
            lastEventAt = event.time;
        const data = event.data;
        switch (event.type) {
            case 'request/header': {
                const config = data?.header;
                const headerConfig = config?.config;
                if (typeof headerConfig?.provider === 'string')
                    provider = headerConfig.provider;
                if (typeof headerConfig?.model === 'string')
            ...
            case 'compaction/summary': {
                ...
                if (!accounted && data?.llmStreamCall !== true)
                    break;
                calls.push({
                    time: event.time,
 ...
 */
export function foldSessionLog(meta, events, range) {
 ...
 */
export function rangeOf(request) {
 ...
 */
export function buildUsageReport(request, path, title, generatedAt, logs) {
    ...
    for (const log of logs) {
        ...
        if (log.lastEventAt > totals.lastEventAt)
            totals.lastEventAt = log.lastEventAt;
        ...
        if (range !== undefined && sessionAcc.apiCalls === 0 && log.toolCalls === 0 && log.turns === 0)
            continue;
        totals.sessions += 1;
...
export class WorkspaceUsage {
    ...
    async report(request, signal) {
        ...
        if (workspace === undefined) {
            throw new RemoteError('usage/workspace-not-found', `cannot build a usage report: unknown workspace "${request.workspaceId}"`, { workspaceId: request.workspaceId });
        }
        const range = rangeOf(request);
        const logs = [];
    ...
    async sessionLog(request, signal) {
        const workspace = this.ctx.workspaceRegistry.get(request.workspaceId);
        if (workspace === undefined) {
            throw new RemoteError('usage/workspace-not-found', `cannot read a session log: unknown workspace "${request.workspaceId}"`, { workspaceId: request.workspaceId });
        }
        if (!workspace.sessionIds.includes(request.sessionId)) {
            throw new RemoteError('usage/session-not-in-workspace', `session "${request.sessionId}" is not accounted to workspace "${request.workspaceId}"`, { workspaceId: request.workspaceId, sessionId: request.sessionId });
        }
        const cold = await this.readStoredLog(request.sessionId, signal);
        if (cold === undefined) {
            throw new RemoteError('usage/session-log-unavailable', `no persisted log exists for session "${request.sessionId}"`, { sessionId: request.sessionId });
        }
