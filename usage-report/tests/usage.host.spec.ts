...
function headerLine(id = SID, createdAt = 1_700_000_000_000): string {
...
function eventLine(type: string, seq: number, time: number, data: unknown): string {
  return JSON.stringify({ type, seq, time, data })
}

/** A small fixture log: one turn, two steps, one tool call, one malformed line. */
function fixtureLog(time = 1_700_000_000_000): string {
  return [
    headerLine(),
    eventLine('turn/start', 1, time, { turn: 1 }),
    eventLine('request/header', 2, time, {
      header: { config: { provider: 'deepseek-official', model: 'deepseek-v4-flash' } },
      reason: 'initial',
    }),
    eventLine('step/start', 3, time, { turn: 1, step: 1 }),
    eventLine('assistant/message', 4, time, {
      turn: 1,
      step: 1,
      message: { role: 'assistant', content: [{ type: 'text', text: 'hello' }] },
      usage: { inputTokens: 100, outputTokens: 20, totalTokens: 120, cacheReadTokens: 40 },
    }),
    eventLine('tool/call', 5, time, { turn: 1, step: 1, callId: 'c1', name: 'bash', arguments: '{}' }),
    eventLine('tool/result', 6, time, { turn: 1, step: 1, message: { role: 'tool', content: 'ok' } }),
    eventLine('request/header', 7, time + 1000, {
      header: { config: { provider: 'deepseek-official', model: 'deepseek-v4-flash' } },
      reason: 'series',
    }),
    eventLine('assistant/message', 8, time + 1000, {
      turn: 1,
      step: 2,
      message: { role: 'assistant', content: [{ type: 'text', text: 'world' }] },
      usage: { inputTokens: 50, outputTokens: 10, totalTokens: 60 },
    }),
    '{"type":"assistant/message","seq":9,"time":1,', // malformed tail
  ].join('\n')
}
...
describe('parseSessionEvents / parseSessionLog', () => {
  it('folds calls, tool calls, turns, and malformed lines', () => {
    const parsed = parseSessionLog({ id: SID, version: 0, createdAt: 1_700_000_000_000 }, fixtureLog())
    ...
    expect(parsed.calls[1]).toMatchObject({ usage: { inputTokens: 50, outputTokens: 10, totalTokens: 60 } })
    expect(parsed.toolCalls).toBe(1)
    expect(parsed.turns).toBe(1)
    expect(parsed.malformedLines).toBe(1)
    expect(parsed.firstEventAt).toBe(1_700_000_000_000)
  })

  it('attributes calls to unknown before any request header', () => {
    const content = [
      headerLine(),
      eventLine('assistant/message', 1, 1_700_000_000_000, {
        turn: 1,
        step: 1,
    ...
    const parsed = parseSessionLog({ id: SID, version: 0, createdAt: 1 }, content)
    expect(parsed.calls[0]).toMatchObject({ provider: 'unknown', model: 'unknown' })
  })
  ...
  it('keeps the attributed route when a header records no config', () => {
    ...
    const content = [
      headerLine(),
      eventLine('request/header', 1, time, {
        header: { config: { provider: 'deepseek-official', model: 'deepseek-v4-flash' } },
        reason: 'initial',
      }),
      eventLine('request/header', 2, time, { reason: 'series' }),
      eventLine('assistant/message', 3, time, {
        turn: 1,
        step: 1,
        message: { role: 'assistant', content: [] },
        usage: { inputTokens: 1, outputTokens: 1 },
    ...
    const parsed = parseSessionLog({ id: SID, version: 0, createdAt: time }, content)
    expect(parsed.calls[0]).toMatchObject({ provider: 'deepseek-official', model: 'deepseek-v4-flash' })
  })

  it('skips blank lines and numbers a seq-less event by position', () => {
    const content = [
      headerLine(),
      '',
      JSON.stringify({ type: 'turn/start', time: 1_700_000_000_000, data: { turn: 1 } }),
      '',
    ].join('\n')
    const { events, malformedLines } = parseSessionEvents(content)
    expect(malformedLines).toBe(0)
    expect(events).toEqual([{ seq: 0, time: 1_700_000_000_000, type: 'turn/start', data: { turn: 1 } }])
  })

  it('counts parseable lines that are not events as malformed', () => {
    const content = [
      headerLine(),
      'null',
      JSON.stringify({ type: 'turn/start', seq: 1 }), // no time
      JSON.stringify({ seq: 2, time: 1_700_000_000_000 }), // no type
      eventLine('turn/start', 3, 1_700_000_000_000, { turn: 1 }),
    ].join('\n')
    const parsed = parseSessionLog({ id: SID, version: 0, createdAt: 1 }, content)
  ...
  it('accounts compaction summaries under the route they recorded', () => {
    ...
    const content = [
      ...
      eventLine('compaction/summary', 2, time, {
        compactionId: 'c1',
        summary: [],
        provider: 'deepseek-official',
        model: 'deepseek-v4-flash',
    ...
    expect(parsed.calls[1]).toMatchObject({ model: 'deepseek-v4-pro' })
  })
  ...
  it('skips an unmarked compaction summary that reports no usage', () => {
    ...
    const content = [
      headerLine(),
      eventLine('compaction/summary', 1, time, {
        compactionId: 'c1',
        summary: [],
        provider: 'template',
        model: 'none',
  ...
  it('counts a seam-marked summary whose adapter reported no usage', () => {
    ...
    const content = [
      headerLine(),
      // A summary whose recorded route did not survive the artifact still
      // counts as one call, attributed to `unknown` like a headerless response.
      eventLine('compaction/summary', 1, time, { compactionId: 'c1', summary: [], llmStreamCall: true }),
    ].join('\n')
    const parsed = parseSessionLog({ id: SID, version: 0, createdAt: time }, content)
    expect(parsed.calls).toEqual([{ time, provider: 'unknown', model: 'unknown' }])
  })

  it('keeps raw events with the recorded sequence', () => {
    const { events, malformedLines } = parseSessionEvents(fixtureLog())
    expect(malformedLines).toBe(1)
    expect(events[0]).toEqual({ seq: 1, time: 1_700_000_000_000, type: 'turn/start', data: { turn: 1 } })
    // The trailing malformed line is skipped; the last valid event is the second response.
    expect(events.at(-1)).toMatchObject({ seq: 8, type: 'assistant/message' })
  })
})
...
describe('buildUsageReport', () => {
  ...
  it('aggregates totals, hours, models, and sessions across logs', () => {
    const meta = { id: SID, version: 0, createdAt: 1_700_000_000_000 }
    const log = parseSessionLog(meta, fixtureLog())
    ...
    expect(report.bySession[0]).toMatchObject({ sessionId: SID, apiCalls: 2, malformedLines: 1 })
  ...
  it('orders hours chronologically, models by cost, and sessions by recency', () => {
    ...
    const unpriced = [
      headerLine(other, time),
      eventLine('request/header', 1, later, {
        header: { config: { provider: 'p', model: 'mystery-a' } },
        reason: 'initial',
      }),
      eventLine('assistant/message', 2, later, {
        turn: 1,
        step: 1,
        message: { role: 'assistant', content: [] },
        usage: { inputTokens: 5, outputTokens: 1 },
      }),
      eventLine('request/header', 3, later, {
        header: { config: { provider: 'p', model: 'mystery-b' } },
        reason: 'series',
      }),
      eventLine('assistant/message', 4, later, {
        turn: 1,
        step: 2,
        message: { role: 'assistant', content: [] },
      ...
      eventLine('assistant/message', 5, later, { turn: 1, step: 3, message: { role: 'assistant', content: [] } }),
    ].join('\n')
    const report = buildUsageReport(request, '/workspace', 'Workspace', later + 500, [
      parseSessionLog({ id: SID, version: 0, createdAt: time }, fixtureLog()),
      parseSessionLog({ id: other, version: 0, createdAt: time }, unpriced),
    ])
  ...
  it('carries the parent session of a subagent child row', () => {
    ...
    const report = buildUsageReport(request, '/workspace', 'Workspace', 1_700_000_000_500, [
      parseSessionLog(
        { id: child, version: 0, createdAt: 1_700_000_000_000, parentSession: parent },
        fixtureLog(),
      ),
    ])
  ...
  it('folds only responses inside the requested window', () => {
    ...
    const report = buildUsageReport(
      ...
      'Workspace',
      1_700_000_000_500,
      [parseSessionLog(meta, fixtureLog(), range)],
    )
  ...
  it('keeps every session when no window is requested', () => {
    ...
    const report = buildUsageReport(
      ...
      'Workspace',
      1_700_000_000_500,
      [parseSessionLog(meta, fixtureLog())],
    )
  ...
  it('flags unknown models as unpriced', () => {
    const content = [
      headerLine(),
      eventLine('request/header', 1, 1_700_000_000_000, {
        header: { config: { provider: 'p', model: 'mystery-model' } },
        reason: 'initial',
      }),
      eventLine('assistant/message', 2, 1_700_000_000_000, {
        turn: 1,
        step: 1,
        message: { role: 'assistant', content: [] },
    ...
    const report = buildUsageReport(
      ...
      'Workspace',
      1_700_000_000_500,
      [parseSessionLog({ id: SID, version: 0, createdAt: 1 }, content)],
    )
...
describe('WorkspaceUsage service', () => {
  function harness(
    readArtifacts: Map<string, { content: string }>,
    live: Map<string, { events: Array<{ time: number; type: string; data: unknown }> }> = new Map(),
  ) {
    ...
    ctx.provide('sessionPersistence', {
      readRaw: vi.fn(async (id: SessionId) => {
        const artifact = readArtifacts.get(id)
        return artifact === undefined
          ? undefined
          : { meta: { id, version: 0, createdAt: 1 }, filename: 'session.jsonl', content: artifact.content }
      }),
    } as never)
    ctx.provide('sessions', {
      get: vi.fn((id: SessionId) => {
        ...
        return session === undefined
          ? undefined
          : { header: { id, version: 0, createdAt: 1 }, events: session.events }
      }),
    } as never)
    return new WorkspaceUsage(ctx as unknown as Context)
  }

  it('reports over persisted artifacts', async () => {
    const usage = harness(new Map([[SID, { content: fixtureLog() }]]))
    const report = await usage.report({ workspaceId: WID, timeZone: 'UTC' }, new AbortController().signal)
    expect(report.totals.apiCalls).toBe(2)
    expect(report.path).toBe('/workspace')
  })

  it('folds live in-memory sessions without reading their artifact', async () => {
    const usage = harness(new Map(), new Map([[SID, {
      events: [
        { time: 1_700_000_000_000, type: 'request/header', data: { header: { config: { provider: 'p', model: 'm' } } } },
        {
          time: 1_700_000_000_000,
          type: 'assistant/message',
          data: { usage: { inputTokens: 5, outputTokens: 3 } },
        },
      ],
    }]]))
    const report = await usage.report({ workspaceId: WID, timeZone: 'UTC' }, new AbortController().signal)
  ...
  it('skips an accounted session with no artifact and no live log', async () => {
  ...
  it('reads a session log and truncates to the newest events', async () => {
    ...
    for (let seq = 1; seq <= MAX_USAGE_LOG_EVENTS + 10; seq++) {
      lines.push(eventLine('turn/start', seq, 1_700_000_000_000 + seq, { turn: seq }))
    }
    const usage = harness(new Map([[SID, { content: lines.join('\n') }]]))
  ...
  it('returns a short log untruncated', async () => {
    const usage = harness(new Map([[SID, { content: fixtureLog() }]]))
    const log = await usage.sessionLog({ workspaceId: WID, sessionId: SID }, new AbortController().signal)
  ...
  it('rejects a session not accounted to the workspace and a missing artifact', async () => {
    const usage = harness(new Map())
