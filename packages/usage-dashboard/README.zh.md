...
该包插入两行：`usage-report`（`@deepseek-ai/dsh-usage-report`，Host 端 `usage` Remote 命名空间及其报告折叠，含 DeepSeek 价格表）与 `ui-usage`（`@deepseek-ai/dsh-client-ui-usage`，浏览器仪表盘）。base 的 workspace 控制器保持不变，因此没有本包的 profile 两者都没有。仪表盘读取持久化会话日志，是只读的。
...
- [usage-report](../../usage/usage-report/README.zh.md) — 本包组合的 Host 端 `usage` Remote 命名空间，提供 `report` 与 `sessionLog`。
...
- **两个半边一起安装** — 补丁把 Host 行与浏览器行作为一个整体组合；profile 无法只取 `usage` Remote 命名空间而不要仪表盘 UI，反之亦然。拆成两个 bundle 推迟到用量报告出现第二个消费方之时。
